...
const int MAX = 3e5 + 5;
ll dp[MAX];
int operation(vector<pi>&bt, string &s, int cur) {
	int l = bt[cur].first;
	int r = bt[cur].second;
	if (l == 0 && r == 0) return 0;
	...
	int op1 = 1e9, op2 = 1e9;
	if (l != 0) {
		op1 = int(s[cur - 1] != 'L') + operation(bt, s, l);
	}
	if (r != 0) {
		op2 = int(s[cur - 1] != 'R') + operation(bt, s, r);
	}
