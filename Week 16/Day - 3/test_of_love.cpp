...
/*Problem Link -> https://codeforces.com/problemset/problem/1992/D*/
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
ll dp[200005];
ll go(int i, string &s, int n, int m, int k) {
	if (i >= n) return 0;
	if (dp[i] != -1) return dp[i];
	if (s[i] == 'C') return INT_MAX;
	if (s[i] == 'W') return dp[i] = 1 + go(i + 1, s, n, m, k);
	ll jumps = INT_MAX;
	for (int j = 1; j <= m; j++) {
		jumps = min(jumps, go(i + j, s, n, m, k));
	}
	return dp[i] = jumps;
}
void solve()
